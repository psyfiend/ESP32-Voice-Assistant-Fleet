...
| 2.6 | Tileview navigation | Swipe L/R between pages, U/D to menus; gesture conflicts resolved (§5.2); page indicator dots. **VERTICAL SWIPES STARTED EARLY on `fix/49-link-liveness`, 2026-09-19** — down from the top-right opens the system drawer, down from the top-left opens the log, up from the bottom shows the deck, and a hidden header peeks for 3 s before a second swipe commits. Edge-gated, and the swipe DISTANCE is `sc(50)` so it is the same physical gesture on every panel rather than LVGL's 50 *physical* px. Horizontal swipes are deliberately unclaimed, because they are this milestone's actual job. See the gesture notes in `GUIManager.cpp` |
