...
**`docs/HANDOFF.md` is the answer to that question and this section is not.** It used to carry a
dated snapshot that went stale within a session and then contradicted the handoff, which is worse
than saying nothing. Current status, what is in flight and what to do next all live there.

What belongs HERE is the plan: what we are building, in what order, and how we know a milestone is
finished. Section 7 is that list.
...
| 2.6 | Tileview navigation | Swipe L/R between pages, U/D to menus; gesture conflicts resolved (§5.2); page indicator dots. **VERTICAL SWIPES LANDED EARLY and MERGED to `main` 2026-09-19 (`7d723be`)**, verified on glass across four boards — down from the top-right opens the system drawer, down from the top-left opens the log, up from the bottom shows the deck, and a hidden header peeks for 3 s before a second swipe commits. Edge-gated, and the swipe DISTANCE is `sc(50)` so it is the same physical gesture on every panel rather than LVGL's 50 *physical* px. Horizontal swipes are deliberately unclaimed, because they are this milestone's actual job. See the gesture notes in `GUIManager.cpp` |
| 2.7 | First 4 card types | `sensor`, `binary_sensor`, `switch`, `button` — bound to real HA entities. **UNBLOCKED 2026-09-20:** #43 put the owner's 18 real entities on glass, so this is judgeable for the first time rather than theoretical. What it still owes, now with evidence: a `door` type (both garage sensors report `device_class: garage_door` and render as generic binary sensors); `LightCard` learning brightness (`light.office` is a group with `['color_temp','xy']`, `light.dining_room_light` is `['brightness']`); the corner-icon/hero split; and consuming HA's live `attributes.icon`, whose glyphs are now in the font but which nothing reads yet. `Entity::lastChangeMs` (#57) exists for "open for 40 minutes" and has no caller. See `docs/design/dashboard-target-7b.md` |
...
| # | Milestone | Acceptance criteria |
|---|---|---|
| 5.1 | Partition layout decision | Finalised **before** the first OTA release (§6.10). See the grid defaults below - a flashable image should boot into the right layout for its board without anyone touching a knob |
...
#### Per-board default grid, chosen on the glass 2026-09-17/18

A board should boot into ITS layout, not into whatever the derivation happens to produce. These are
the owner's picks after turning the knobs on real hardware, and they belong in the flashed image -
`5.1`'s images and `3.2`'s compiled sheets should both honour them. The Col/Row knobs stay as an
override.

| Board | Default | Notes |
|---|---|---|
| `CYD_S3_3248` | **2x4** | 2x3 optional. It will never want anything else, spans aside |
| `WS_S3_4B`, `WS_P4_4B` | **3x4** | Reads better than 4x3. 4x4 is cramped; 3x3 may suit the slower S3. Wants more experimentation, 3x4 is the happy medium |
| `WS_P4_7B`, `CYD_P4_1060` | **6x3** | |
| `WS_P4_5` | **5x3** | |

**Both 4B boards REQUIRE a font-size reduction** - hero and name - in every grid arrangement. The
mechanism exists: `scripts/gen_type_scale.py` has a per-board `SCALE` multiplier, already used to
take `WS_P4_5` to 0.88. Neither 4B has been given one yet.

The knobs that produced these are `TARGET_CARD_W` (columns, per board in `UITokens.cpp`) and the
`CardPage` row override. Both are documented in `docs/design/dashboard.md` §6.
