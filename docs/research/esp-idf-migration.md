...
- **PPA** (Picture Processing Accelerator, P4's 2D hardware block) — **not actually gated behind this migration.** Verified in `docs/REFERENCE_PROJECTS.md`: the `ESP32-P4-Allsky-Display` reference project runs a working `PPAAccelerator` class over `driver/ppa.h` **from Arduino framework**, because arduino-esp32 3.x exposes ESP-IDF driver headers directly. Migrating to ESP-IDF would not newly unlock PPA; it's already reachable. `docs/research/display-stack-migration.md` (a separate, narrower investigation into this exact question) independently confirmed it further: PPA is gated by `SOC_PPA_SUPPORTED` (defined on P4, undefined on S3) and only the P4 package variants link `-lesp_driver_ppa` — so it's specifically P4-only silicon, not a framework limitation either way. That same report also found LVGL 9.5 (already vendored here) ships its own PPA draw unit, currently disabled (`LV_USE_PPA 0`), but it explicitly refuses to accelerate anything with `rotation != 0` — so turning it on would speed up fills/image blends but would **not**, by itself, fix the whole-framebuffer rotation stutter CPU-side rotation currently causes; that would still need hand-written code calling `ppa_do_scale_rotate_mirror()` directly, on Arduino or IDF alike.
...
|---|---|---|---|
| `GFX_Library_for_Arduino` (fork) | All display output | `esp_lcd` + per-panel driver components + `esp_lvgl_port`/`esp_lvgl_adapter` | **Large, but smaller than first estimated — see `docs/research/display-stack-migration.md`.** This is the migration's critical path — see §1.1. Of the three panel families, RGB and DSI are "delete a thin wrapper" (Arduino_GFX's own RGB/DSI backends already call `esp_lcd_new_rgb_panel()`/`esp_lcd_dsi_bus` directly, verified there by reading `Arduino_ESP32RGBPanel.cpp`/`Arduino_DSI_Display.cpp`), and Espressif's own Apache-2.0 `ESP32_Display_Panel` already has per-chip drivers for 7 of 8 boards, AXS15231B (QSPI) included. The one confirmed gap needing real driver-writing effort is `WS_P4_5`'s HX8394. |
