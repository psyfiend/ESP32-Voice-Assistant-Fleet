 ...
 * Vendored from waveshare/esp_lcd_hx8394 2.1.0 (MIT, LICENSE_esp_lcd_hx8394.txt).
 * Two changes, and only these two:
 *
 * 1. The board-specific I2C sequence DEFAULTS TO SKIPPED here (upstream: not
 *    skipped). Left on, the constructor opens I2C bus 1 on pins 7/8 with IDF's
 *    LEGACY I2C driver and writes a device at 0x45 - pins 7/8 are this board's
 *    shared bus, and the legacy driver cannot be linked beside the i2c_master
