...
#ifdef ARDUINO
    // Routed through FleetI2C instead of _myWire directly - see FleetI2C.h.
    // Its begin() is idempotent, so this is safe even when something else
    // (e.g. DisplayManager) already brought the bus up.
    (void)_myWire;
    FleetI2C::begin(iSDA, iSCL, u32Speed);
#else
#ifdef __LINUX__
char filename[32];
int iChannel = iSDA;
    ...
    // TMA445_ADDR (0x24) is identical to CH422G's WR_SET pseudo-address -
    // on any board using our CH422G expander, this probe will ALWAYS get a
    // real ACK from the expander (not a false positive - CH422G genuinely
    // answers there), get misidentified as a TMA445 touch chip, and then
    // send it a garbage "soft reset + security key" sequence meant for a
    // real TMA445. This was the actual root cause of this board's touch
    // never working, discovered after a full day chasing driver-generation
    // theories that were all red herrings - see
    // docs/BRINGUP_WS_S3_TOUCH_LCD_5B.md. Skipped entirely on CH422G boards.
    #ifndef HAS_CH422G
...
//       }
    ...
    if (_iType == CT_TYPE_GT911) { // reset the sensor to start it
       // iRST/iINT can each independently be -1 (unused/not a native pin -
       // e.g. WS_S3_TOUCH_LCD_5B's TP_RST lives behind an IO expander, not a
       // native GPIO). Guarded the same way every other controller type in
       // this function already guards its iINT calls.
       if (iRST != -1) pinMode(iRST, OUTPUT);
       if (iINT != -1) pinMode(iINT, OUTPUT);
       if (iINT != -1) digitalWrite(iINT, LOW);
       if (iRST != -1) digitalWrite(iRST, LOW);
       delay(5);
       if (iINT != -1) digitalWrite(iINT, LOW); // set I2C addr to ADDR1
       delay(1);
       if (iRST != -1) digitalWrite(iRST, HIGH); // when it comes out of reset, it samples INT
       delay(10);
       if (iINT != -1) digitalWrite(iINT, LOW);
       delay(50);
       if (iINT != -1) pinMode(iINT, INPUT);
...
#ifdef ARDUINO
       // No FleetI2C::end() - tearing down a shared bus here isn't safe
       // when something else (e.g. DisplayManager) may still be using it.
#else
#ifdef __LINUX__
       close(_file_i2c);
#else
    ...
    #ifdef DEBUG_TOUCH
    Serial.printf("[bb_captouch] Detected _iType=%d (GT911=%d, SPD2010=%d, TMA445=%d, MXT144=%d, CHSC6540=%d, CST328=%d, CST226=%d, FT6X36=%d, CST820=%d)\n",
        _iType, CT_TYPE_GT911, CT_TYPE_SPD2010, CT_TYPE_TMA445, CT_TYPE_MXT144, CT_TYPE_CHSC6540, CT_TYPE_CST328, CT_TYPE_CST226, CT_TYPE_FT6X36, CT_TYPE_CST820);
    #endif
    return CT_SUCCESS;
} /* init() */
...
#ifdef ARDUINO
    FleetI2C::beginTransmission(u8Addr);
    FleetI2C::write(pData, (uint8_t)iLen);
    rc = !FleetI2C::endTransmission();
#else
#ifdef __LINUX__
...
int BBCapTouch::I2CReadRegister16(uint8_t u8Addr, uint16_t u16Register, uint8_t *pData, int iLen)
{
  ...
  if (_iType == CT_TYPE_MXT144) { // little endian
    FleetI2C::write((uint8_t)u16Register); // low byte
    FleetI2C::write((uint8_t)(u16Register>>8)); // high byte
  } else { // big endian address
    FleetI2C::write((uint8_t)(u16Register>>8)); // high byte
    FleetI2C::write((uint8_t)u16Register); // low byte
  }
  FleetI2C::endTransmission();
  FleetI2C::requestFrom(u8Addr, (uint8_t)iLen);
  while (i < iLen)
  {
      pData[i++] = FleetI2C::read();
  }
#else
    uint8_t ucTemp[4];
    int rc;
...
#ifdef ARDUINO
  FleetI2C::beginTransmission(u8Addr);
  FleetI2C::write(u8Register);
  FleetI2C::endTransmission();
  FleetI2C::requestFrom(u8Addr, (uint8_t)iLen);
  while (FleetI2C::available() && i < iLen)
  {
      pData[i++] = FleetI2C::read();
  }
#else
...
//      Serial.printf("I2CReadRegister returned %d\n", i);
    ...
    if (_iType == CT_TYPE_FT6X36) {
      ...
      #ifdef DEBUG_TOUCH
      // Idle reads are 0x00 (buffer-not-ready bit clear); only log when
      // something's actually happening (a touch event) rather than every
      // idle poll.
      if (ucTemp[0] != 0x00) {
          Serial.printf("[GT911raw] status byte: 0x%02X\n", ucTemp[0]);
      }
      #endif
