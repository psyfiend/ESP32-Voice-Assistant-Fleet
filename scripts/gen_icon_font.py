...
#
# MD, added at 2.7 (cards.md section 13, #62): the icons now step with the
# CARD, not only with the board. One face in between does two jobs - the corner
# icon on a large card, where 2.66 mm looked lost (the owner, on the 7B), and
# the hero on a cramped card, where 4.50 mm will not fit its disc. It is the
# full glyph set rather than a corner-only subset so that a build-sheet corner
# override draws at any size. Measured cost of a face at these sizes: 10-73 KB,
# not the ~96 KB of the note above.
SIZES_MM = {"LG": 4.50, "MD": 3.50, "SM": 2.66}
...
GLYPHS = [
    ...
    # --- named by the owner's own HA entities, 2026-09-20 ------------------
    #
    # Not guessed. scripts/scan_ha_icons.py read these off the live instance:
    # every one is an icon HA is currently serving for an entity on the real
    # dashboard, and without them those cards render tofu rather than a glyph.
    "ceiling-light-outline",   # switch.tv_room_switch_1
    "light-recessed",          # three of the kitchen/office lights
    "coach-lamp",              # light.porch_switch_1
    "garage",                  # both garage door sensors

    # The open half of the garage pair. HA is NOT currently sending it: the
    # owner has a static registry override of mdi:garage on both doors, which
    # suppresses the state variant he actually wants. Shipped anyway, because
    # the fix is one setting in HA and regenerating fonts is the expensive
    # step - see dashboard-target-7b.md.
    "garage-open",
...
def _glyph_stamp(ranges):
    """A short digest of the exact glyph set a face was built from."""
    return hashlib.sha1(",".join(str(r) for r in ranges).encode()).hexdigest()[:12]
...
def generate(px, ranges):
    ...
    stamp = _glyph_stamp(ranges)
    marker = "fleet-glyph-set: " + stamp

    # THE CACHE MUST KNOW WHAT IT CACHED.
    #
    # This used to skip on `os.path.exists(out)` alone, which is wrong in the
    # one way that matters: adding a glyph to GLYPHS and re-running rewrote
    # UIIcons.h with the new NAME, kept the old BITMAPS, and reported "have
    # fleet_icons_18" as though it had done the work. The card then asks for a
    # codepoint the face does not contain and draws tofu - no build error, no
    # warning, and the same silent-failure shape as the missing ':' and 'd' in
    # the numeric subset (#51).
    #
    # Caught 2026-09-20 while adding the owner's five real HA icons: the header
    # gained all five and not one pixel of the fonts changed.
    if os.path.exists(out):
        with io.open(out, "r", encoding="utf-8", errors="ignore") as f:
            head = f.read(2048)
        if marker in head:
            print("    have %s (glyph set unchanged)" % sym)
            return sym
        print("    REBUILD %s - the glyph set changed" % sym)
    ...
    # Stamp the glyph set into the file so the next run can tell whether this
    # face was built from the current GLYPHS list. See the note in generate().
    with io.open(out, "r", encoding="utf-8", errors="ignore") as f:
        body = f.read()
    with io.open(out, "w", encoding="utf-8", newline="") as f:
        f.write("/* " + marker + " - generated by scripts/gen_icon_font.py."
                " Do not edit. */" + chr(10))
        f.write(body)
...
def main():
    ...
    boards = parse_boards()
    sizes = {}
    print("%-14s %5s  %s" % ("BOARD", "PPI", "ICON px  LG / MD / SM"))
    print("-" * 52)
    for b in boards:
        b["px"] = {k: px_for(b["ppi"], mm) for k, mm in SIZES_MM.items()}
        print("%-14s %5d  %11d / %d / %d" % (b["macro"], b["ppi"], b["px"]["LG"],
                                            b["px"]["MD"], b["px"]["SM"]))
    ...
    L.append("//")
    L.append("// The fleet's Material Design Icons subset: %d glyphs, at three sizes" % len(GLYPHS))
    ...
    for b in boards:
        ...
        L.append("    #define FLEET_ICONS_MD (&%s)" % sizes[b["px"]["MD"]])
    ...
    L.append("// are meaningless to any other font - which is why CardIcons.cpp must")
