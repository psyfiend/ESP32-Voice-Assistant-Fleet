// <--- UPDATED: Added Toast Logic & Integrated with Controls
...
void toggle_panel(lv_obj_t * panel, bool &expanded) {
    ...
    // Header is the 1st child [0] of the Panel
    // Icon is the 2nd child [1] of the Header (Label is 0)
...
static void toast_timer_cb(lv_timer_t * t) {
    ...
    // Note: We don't need to delete the timer explicitly if we use one-shot,
    // but lv_timer_del inside callback is generally safe in v8+.
    // However, to be safe and reusable, we just mark handle null.
    // Actually, let's just let it be deleted by the system if we used one-shot.
    // But since we want to reuse the handle logic, we'll just ignore it.
...
static void sw_enable_cb(lv_event_t * e) {
    ...
    if (state) {
        ...
        lv_obj_remove_state(bar_vu_l, LV_STATE_DISABLED);
        lv_obj_remove_state(bar_vu_r, LV_STATE_DISABLED);
        ...
        lv_obj_add_state(bar_vu_l, LV_STATE_DISABLED);
        lv_obj_add_state(bar_vu_r, LV_STATE_DISABLED);
...
void process_audio_test_sequence() {
    ...
    if (rec_buffer == NULL) {
        ...
        // <--- UPDATED: Correct Buffer Size for STEREO (2 Channels)
        // Size = SampleRate * Channels * BytesPerSample * Seconds
        // 16000 * 2 * 2 * 4 = 256,000 bytes
        rec_buffer_size = 16000 * 2 * 2 * (int)REC_LIMIT_SECONDS; 
        rec_buffer = (int16_t*)heap_caps_malloc(rec_buffer_size, MALLOC_CAP_SPIRAM); // Alloc in PSRAM
        
        if (rec_buffer == NULL) {
            Serial.println("[UI] Failed to allocate audio buffer!");
            lv_label_set_text(lbl_pcm_text, "NO RAM!");
            show_toast("Alloc Failed!", 5000);
            return;
        }
    }
    ...
    // --= Correct Sample Count for Stereo =--
    // Total Samples = Rate * Channels * Seconds
    ...
    while(samples_read < samples_total) {
        ...
        if (read > 0) {
            // --= DIGITAL GAIN BOOST =--
            // Boost volume by 4x (+12dB) to compensate for quiet MEMS mics
            // Doing this immediately during capture saves a separate pass
            for (size_t i = 0; i < read; i++) {
                int32_t val = (int32_t)rec_buffer[samples_read + i];
                val = val * 4; // Multiplier: 4x is usually safe for speech
    ...
    if (!timeout) {
        ...
        if (avg_vol < 20.0) { // arbitrary silence threshold
            ...
            show_toast("Silence Recorded?", 5000);
        } else {
...
// --= Touch Point Panel (Created 2nd, so it is below Bottom Deck) =--
...
// --= AUDIO PANEL =--
    ...
    // ROW 1: 1,0 - Switch/Enable Column
    ...
    // ROW 1 COL 2: 2,0 - Volume Slider Column
...
// --= DISPLAY PANEL =--
    ...
    // ROW 1 COL 1: 1,0 - Brightness Slider Column
    ...
    // ROW 1 COL 1: 1,1 - Brightness Label
    ...
    // ROW 1 COL 1: 1,2 - Brightness Slider
    ...
    // ROW 2: 1,0 - Touches Visualization Switch
...
void loop() {
    ...
    if (audio_expanded) {
        ...
        // --= Simple Smoothing / Decay for visual comfort =--
        static int smooth_vol = 0;
        if (mic_vol > smooth_vol) {
            smooth_vol = mic_vol; // Attack fast
        } else {
            smooth_vol = (smooth_vol * 90 + mic_vol * 10) / 100; // Decay slow (90% old, 10% new)
        }
